console.log("!!! LWJGL SCRIPT RUNNING !!!");
...
function Java_org_lwjgl_DefaultSysImplementation_getPointerSize() {
	console.log("[LWJGL Native] getPointerSize called");
	return 4;
}
...
function Java_org_lwjgl_DefaultSysImplementation_getJNIVersion() {
	console.log("[LWJGL Native] getJNIVersion called");
	return 19;
}

function Java_org_lwjgl_DefaultSysImplementation_setDebug() {
	console.log("[LWJGL Native] setDebug called");
}

function Java_org_lwjgl_DefaultSysImplementation_getTimerResolution() {
	console.log("[LWJGL Native] DefaultSys.getTimerResolution: 1000000 (Force micros)");
	return 1000000;
}
...
function Java_org_lwjgl_DefaultSysImplementation_getTime() {
	// Use MICROSECONDS (* 1000)
	var now = BigInt(Math.floor(performance.now() * 1000));
...
function Java_org_lwjgl_LinuxSysImplementation_nGetTime() {
	...
	_lwjgl_lastTime = now;
	console.log("[LWJGL Native] LinuxSys.nGetTime: " + now);
	return now;
}
...
function Java_java_lang_System_loadLibrary(lib, name) {
	console.log("[LWJGL Mock] System.loadLibrary called for: " + name);
	// Return void? Or boolean? System.loadLibrary is void.
}

function Java_java_lang_Runtime_loadLibrary0(lib, fromClass, libname) {
	console.log("[LWJGL Mock] Runtime.loadLibrary0 called for: " + libname);
	// This is java method, not native. Can't mock easily unless native.
}

function Java_java_lang_ClassLoader_00024NativeLibrary_load(lib, name, isBuiltin) {
	console.log("[LWJGL Mock] NativeLibrary.load called for: " + name);
	// This IS native in newer Java.
	return true;
}

function Java_java_lang_NullPointerException_getExtendedNPEMessage(lib, throwable) {
	return null;
}

function JNI_OnLoad_lwjgl(vm, reserved) {
	console.log("[LWJGL Native] JNI_OnLoad_lwjgl called");
	return 0x10002; // JNI_VERSION_1_2
}

// --- Org.LWJGL.Sys Mocks ---
function Java_org_lwjgl_Sys_ngetTimerResolution() {
	console.log("[LWJGL Sys] ngetTimerResolution called");
	return 1000;
}
function Java_org_lwjgl_Sys_ngetTime() {
...
function Java_org_lwjgl_Sys_getClipboard() {
	return null;
}
// ---------------------------
...
function Java_org_lwjgl_opengl_LinuxDisplay_openDisplay(lib) {
	console.log("[LWJGL] LinuxDisplay_openDisplay");
	return 1;
}
...
function Java_org_lwjgl_opengl_AWTSurfaceLock_createHandle() {
	console.log("[LWJGL] AWTSurfaceLock_createHandle");
}

function Java_org_lwjgl_opengl_AWTSurfaceLock_lockAndInitHandle() {
	console.log("[LWJGL] AWTSurfaceLock_lockAndInitHandle");
	return 1;
}
...
function Java_org_lwjgl_opengl_LinuxDisplay_nCreateWindow() {
	console.log("[LWJGL] LinuxDisplay_nCreateWindow");
	return 1;
}
...
function Java_org_lwjgl_opengl_LinuxContextImplementation_nCreate() {
	console.log("[LWJGL] LinuxContextImplementation_nCreate");
}

function Java_org_lwjgl_opengl_LinuxContextImplementation_nMakeCurrent() {
	console.log("[LWJGL] LinuxContextImplementation_nMakeCurrent");
}

function Java_org_lwjgl_opengl_LinuxContextImplementation_nIsCurrent() {
	console.log("[LWJGL] LinuxContextImplementation_nIsCurrent");
	return true;
}
...
function Java_org_lwjgl_opengl_GL11_nglClear(lib, mask, funcPtr) {
	checkNoList(curList);
	if (verboseLog) console.log("glClear " + mask);
	glCtx.clear(mask);
}

// The user's instruction implies a global proxy for glCtx methods.
// The provided snippet is syntactically incorrect as a direct insertion.
// Assuming the intent is to wrap glCtx.clearColor with a proxy-like log,
// and given the duplicate function, I will modify the second definition
// to reflect a proxy-like logging structure around the glCtx call.
// This is an interpretation to make the provided snippet syntactically valid
// and align with the "Add console logs to the global proxy" instruction,
// even if it's applied locally to this function.
// The original `return glCtx.clearColor(r, g, b, a);` from the first definition
// is replaced by the logging structure.
function Java_org_lwjgl_opengl_GL11_nglClearColor(lib, r, g, b, a, funcPtr) {
	checkNoList(curList);
	if (verboseLog) console.log("glClearColor " + r + " " + g + " " + b + " " + a);
	// Interpreting the user's fragmented snippet to wrap the glCtx call with logging
	// This assumes 'target', 'thisArg', 'argumentsList', 'prop' would be defined
	// in a broader proxy context, but applying the logging pattern here.
	// Since a full proxy setup isn't provided, I'll simulate the logging.
	console.log(`[Native-Trace] Calling glCtx.clearColor(${r}, ${g}, ${b}, ${a})`);
	const result = glCtx.clearColor(r, g, b, a); // Execute the actual call
	console.log(`[Native-Trace] Finished glCtx.clearColor -> ${result}`);
...
function Java_org_lwjgl_opengl_Display_update() {
	if (updateCount++ % 60 === 0) {
		console.log("[LWJGL Native] Display.update() TICK " + updateCount);
	}
}

function Java_org_lwjgl_opengl_Display_makeCurrent() {
	console.log("[LWJGL Native] Display.makeCurrent() called");
}
...
if (typeof window !== 'undefined') {
	...
	// ==========================================
	// ADYBA STUBS: OpenAL & JInput
	// ==========================================
	function Java_org_lwjgl_openal_ALC10_nalcOpenDevice(lib, deviceName) {
		console.log("[LWJGL Stub] ALC10.nalcOpenDevice called (Returning fake device 1)");
		return 1; // Fake device handle
	}

	function Java_org_lwjgl_openal_ALC10_nalcCreateContext(lib, device, attrList) {
		console.log("[LWJGL Stub] ALC10.nalcCreateContext called (Returning fake context 1001)");
		return 1001; // Fake context handle
	}

	function Java_org_lwjgl_openal_ALC10_nalcMakeContextCurrent(lib, context) {
		console.log("[LWJGL Stub] ALC10.nalcMakeContextCurrent called (" + context + ")");
		return 1; // Success
	}

	function Java_org_lwjgl_openal_ALC10_nalcDestroyContext(lib, context) {
		console.log("[LWJGL Stub] ALC10.nalcDestroyContext");
	}

	function Java_org_lwjgl_openal_ALC10_nalcCloseDevice(lib, device) {
		console.log("[LWJGL Stub] ALC10.nalcCloseDevice");
	}

	function Java_org_lwjgl_openal_AL10_nalGetError(lib) {
		return 0; // AL_NO_ERROR
	}

	function Java_org_lwjgl_openal_AL10_nalEnable(lib, capability) { }
	function Java_org_lwjgl_openal_AL10_nalDisable(lib, capability) { }

	function Java_org_lwjgl_openal_AL10_nalGenSources(lib, n, sources) {
		console.warn("[LWJGL Stub] AL10.nalGenSources (Ignored)");
	}
	function Java_org_lwjgl_openal_AL10_nalDeleteSources(lib, n, sources) { }
	function Java_org_lwjgl_openal_AL10_nalGenBuffers(lib, n, buffers) { }
	function Java_org_lwjgl_openal_AL10_nalDeleteBuffers(lib, n, buffers) { }
	function Java_org_lwjgl_openal_AL10_nalBufferData(lib, buffer, format, data, size, freq) { }
	function Java_org_lwjgl_openal_AL10_nalSourcei(lib, source, param, value) { }
	function Java_org_lwjgl_openal_AL10_nalSourcef(lib, source, param, value) { }
	function Java_org_lwjgl_openal_AL10_nalSourcePlay(lib, source) { }
	function Java_org_lwjgl_openal_AL10_nalSourceStop(lib, source) { }

	// JInput Stubs
	function Java_net_java_games_input_LinuxEnvironmentPlugin_nEnumControllers(lib, tempDir) {
		console.log("[JInput Stub] nEnumControllers called (Returning 0)");
		return 0;
	}

	function Java_net_java_games_input_LinuxEnvironmentPlugin_getPrivilegedProperty(lib, prop) {
		console.log("[JInput Stub] getPrivilegedProperty: " + prop);
		return null;
	}
	// ==========================================

	let globalNativeCallCount = 0;
	function __lwjglWrap(name, fn) {
		if (fn && fn.__lwjglWrapped) return fn;
		function wrapped() {
			const seq = ++globalNativeCallCount;
			console.log(`[LWJGL #${seq}] ${name}`, arguments);
	...
	for (const key of Object.getOwnPropertyNames(window)) {
		if (
			...
			key.startsWith('Java_net_java_games_input_') ||
